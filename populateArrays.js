...
if (query)
{
	while (talent[talentInsertID])
	{
		...
		if (talent[talentInsertID][3] != 1)	rankTop[talentInsertID] = [savedRank, rank[talentInsertID][savedRankCurrent], rank[talentInsertID][savedRankNext]];	
	...
	while (q < talent.length)
	{
		if (talent[q][6]) hasDependentTalents[talent[q][6][0]]++;
		q++;
	}
...
else
{
	while (talent[talentInsertID])
	{
		minLevel[talentInsertID] = getMinLevel(talentInsertID); //fill in the minLevel array
		rankBottom[talentInsertID] = [1, rank[talentInsertID][0]];	
		if (talent[talentInsertID][3] != 1)	rankTop[talentInsertID] = [0, rank[talentInsertID][0], rank[talentInsertID][1]];	
	...
	{
		if (talent[q][6])
		hasDependentTalents[talent[q][6][0]]++;
		q++;
	}
}
