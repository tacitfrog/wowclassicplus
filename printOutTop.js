...
for (var printTreeID = 0; printTreeID < numberOfTrees; printTreeID++) {
	...
	talentsReplaceString += '<td  style = "background: url(\'' + nlclass + '_files/background' + printTreeID + '.jpg\') #000000 no-repeat; background-size: 100% 100%;">';	
		...
		for (verticalCounter = 1; verticalCounter <= vertical; verticalCounter++)
		{
			...
			for (horizontalCounter = 1; horizontalCounter <= horizontal; horizontalCounter++)
			{
				...
				if (talent[i] && talent[i][3] == horizontalCounter && talent[i][4] == verticalCounter)
				{
					...
					talentsReplaceString += rankTop[i][0] +'</span>/'+ talent[i][2] +'</span></td></tr></table></div></div>';
					...
					talentsReplaceString += '<br><span class = "rankTooltip">' + textRankColon +' <span id = "modifyRankTopDescription' + i +'" class = "rankTooltip">' + rankTop[i][0] + '</span>/' + talent[i][2] + '</span>';
					...
					{ 
						talentsReplaceString += '<span id = "requiresPoints'+ i +'" style = "color: red" class = "mySmall"><br>' + requiresRequires + ' ' + ((talent[i][4]*5)-5) + ' ' + requiresPointsIn + ' ' + tree[talent[i][0]] + ' ' + requiresTalents +'</span>';
					}
					...
					talentsReplaceString += '<div style = "z-index: 50; position: relative;"><div style = "top: -2px; left: -2px; position: absolute;"><img src = "img/icon-over-grey.gif" border = "0" /></div></div>';
					
					//This has to do with "if the talent has points
					if (rankTop[i][0] >= 1)
						talentsReplaceString += '<div id = "iconOverGreen' + i + '" style = "position: relative; z-index:50;"><div id = "iconOver"><img src = "img/icon-over-green.gif" border = "0" /></div></div>';
					else
						talentsReplaceString += '<div id = "iconOverGreen' + i + '" style = "visibility:hidden; position: relative; z-index:50;"><div id = "iconOver"><img src = "img/icon-over-green.gif" border = "0"></div></div>';

					//This is if the talent has max points
					if (rankTop[i][0] == talent[i][2])
						talentsReplaceString += '<div id = "iconOverYellow' + i + '" style = "position: relative; z-index:50;"><div id = "iconOver"><img src = "img/icon-over-yellow.gif" border = "0"></div></div>';
					else 
						talentsReplaceString += '<div id = "iconOverYellow' + i + '" style = "visibility:hidden; position: relative; z-index:50;"><div id = "iconOver"><img src = "img/icon-over-yellow.gif" border = "0"></div></div>';
					
					//This is if the talent can't have points..?
					if (rankTop[i][0] == 0)							
						talentsReplaceString += '<div id = "talentThumb' + i + '" style = "visibility: hidden; position: relative;"><div id = "wrapper49"><img src = "' + nlclass + '_files/' + imageName + '.jpg" border = "0" '+ iconSize +'></div></div>';
					else
						talentsReplaceString += '<div id = "talentThumb' + i + '" style = "position: relative"><div id = "wrapper49"><img src = "' + nlclass + '_files/' + imageName + '.jpg" border = "0" '+ iconSize + '></div></div>';
					...
					if (talent[i][5]) {
						...
						talentsReplaceString += '<img src = "' + nlclass + '_files/' + imageName + '-off.jpg" border = "0" ' + iconSize +'>';
					i++;
				}
				else
					talentsReplaceString += "&nbsp;";
...
talentsReplaceString += '<div id = "hideBoxBot"';

if (!variableIsSite)
	talentsReplaceString += ' style = "display: none;"';

talentsReplaceString += '>';

talentsReplaceString += '<p>'+ textToSave +'</p>';

talentsReplaceString += '<p id="copyURL"></p>';

talentsReplaceString += '<p class="sourceCredits">'+ textToSource +'</p></div>';

document.getElementById('replaceMeWithHeader').innerHTML = headerReplaceString;
document.getElementById('replaceMeWithTalents').innerHTML = talentsReplaceString;
